...
export function UserProfile() {
  ...
  if (isPending) return <div className="size-8 rounded-full bg-base-200 animate-pulse" />;
  ...
  return (
    <div className="flex flex-row items-center gap-3 md:gap-4 z-50">
      <DropdownMenu>
        <DropdownMenuTrigger asChild className="shadow-none ring-0 border-0 outline-0">
        ...
        >
          <div className="flex items-center gap-2 px-2 py-2 text-left text-sm">
            <Avatar className="h-8 w-8 rounded-[14px]">
              ...
              <AvatarFallback className="rounded-[13px]">{user.name?.slice(0, 2).toUpperCase() || "SI"}</AvatarFallback>
            </Avatar>
